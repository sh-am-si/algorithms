...
class PercolationGraphic:
    def __init__(self, num, p) -> None:
        ...
        self.array[
            np.random.choice(
                np.arange(self.num**2), int(self.p * self.num**2), replace=False
            )
        ] = True
...
class Percolation:
    def __init__(self, num, none_zero_num=0) -> None:
        ...
        self.array = np.zeros(shape=(self.num * self.num,), dtype=bool)
        self.array[
            np.random.choice(np.arange(self.num**2), self.none_zero_num, replace=False)
        ] = True
        self.array = self.array.reshape(self.num, self.num)
    ...
    def percolates(self, alg) -> bool:
        ...
        if (
            self.array[self.num - 1, self.num - 1]
            and self.array[self.num - 2, self.num - 1]
        ):
            algorithm.union(
                get_index(self.num - 1, self.num - 1),
                get_index(self.num - 2, self.num - 1),
            )
